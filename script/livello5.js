...
function create() {
    ...
    aereo = this.physics.add.staticSprite(1400, floorY - 10, 'aereo');
    aereo.setOrigin(0.5, 1).setScale(9).refreshBody(); 
...
function triggerMainDialogue() {
    startDialogue([
        `Hold on, ${playerName.toUpperCase()}! Before proceeding, you must know what we are about to do.`,
        `Welcome to the NASA Zero-G program!`,
        `That huge plane you see behind me is the famous "Vomit Comet".`,
        `We use it to simulate the weightlessness of space without leaving Earth!`,
        `It's not magic, it's physics: the plane enters a high-speed dive.`,
        `During this maneuver, for 20 seconds, you'll float inside just like in space!`,
        `Go on, approach the plane and press [E] to board and start the practical test!`
...
function update() {
    ...
    if (keys.A.isDown) { player.setVelocityX(-SETTINGS.walkSpeed); player.setFlipX(true); player.play('astro_walk', true); } 
    else if (keys.D.isDown) { player.setVelocityX(SETTINGS.walkSpeed); player.setFlipX(false); player.play('astro_walk', true); } 
    ...
    let distAereo = Phaser.Math.Distance.Between(player.x, player.y, aereo.x, aereo.y);
    ...
    if (distNpc < 200) {
        ...
        if (Phaser.Input.Keyboard.JustDown(interactKey) && !isDialogOpen) {
            if (!hasTalkedToScientist) { triggerMainDialogue(); } else { startDialogue([ `Remember Recruit: approach that giant plane and press [E] to board. Have a great flight!` ]); }
        }
    } 
    else if (distAereo < 400 && hasTalkedToScientist) {
        this.interactPrompt.setPosition(aereo.x, aereo.y - 450).setText("[E] ENTER").setVisible(true);
        if (Phaser.Input.Keyboard.JustDown(interactKey)) {
            isTransitioning = true; player.setVelocityX(0).play('astro_idle', true);
            document.getElementById('fade-overlay').style.opacity = '1';
            setTimeout(() => { window.location.href = 'aereo.html'; }, 1000);
        }
    }
    else if (distDoor < 150) { this.interactPrompt.setPosition(portalEnd.x, portalEnd.y - 250).setText("LOCKED").setVisible(true); }
}